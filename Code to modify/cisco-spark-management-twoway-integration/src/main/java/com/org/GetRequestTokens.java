package com.org;

import com.atlassian.oauth.client.example.CostumeClass;;

public class GetRequestTokens {
	public String getRequestTokenFromJar(String newVariable){
		CostumeClass oauth = new CostumeClass();
		String[] args=new String[10];
		//Consumer Key
		args[0]="hardcoded-consumer";
		
		//Consumer Private Key
		args[1]="";
		
		args[2]="requestToken";
		
		//Base Url
		args[3]="https://jirademotest.atlassian.net";
		String[] requestToken = oauth.getRequestTokenForJira(args);
		String responseJson = "{\"requestToken\":\""+requestToken[0]+"\",\"secretToken\":\""+requestToken[1]+"\",\"requestUrl\":\""+requestToken[2]+"\"}";
		return responseJson;
	}
	
	
	public static void main(String[] args) {
		CostumeClass oauth = new CostumeClass();
		args=new String[10];
		//Consumer Key
		args[0]="hardcoded-consumer";
		
		//Consumer Private Key
		args[1]="";
		
		args[2]="requestToken";
		
		//Base Url
		args[3]="https://jirademotest.atlassian.net";
		String[] requestToken = oauth.getRequestTokenForJira(args);
		String responseJson = "{\"requestToken\":\""+requestToken[0]+"\",\"secretToken\":\""+requestToken[1]+"\",\"requestUrl\":\""+requestToken[2]+"\"}";
		System.out.println(responseJson);
	}
}
